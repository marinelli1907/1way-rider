...
import { MapPin, Star, Clock, DollarSign, User, ArrowRight, CheckCircle, Users, Gavel, Zap, Dog, Car, Baby, AlertCircle, Filter, Info, Timer, X } from 'lucide-react-native';
...
import { AppliedCoupon, BidFilterType } from '@/types';
...
export default function SelectDriverScreen() {
  ...
  const [bidFilter, setBidFilter] = useState<BidFilterType | null>(null);
  const [showPolicyModal, setShowPolicyModal] = useState(false);

  const basePrice = parseFloat(params.basePrice || '0');

  const pickupDate = new Date(params.pickupTime);
  const hoursUntilRide = (pickupDate.getTime() - Date.now()) / (1000 * 60 * 60);

  const event = events.find(e => e.id === params.eventId);
  const connectedProfiles = getConnectedProfiles();
  const eventConnections = event?.invitedProfiles?.length || 0;

  React.useEffect(() => {
    console.log('Event connections changed:', eventConnections);
    console.log('Event invited profiles:', event?.invitedProfiles);
    if (event && event.invitedProfiles && event.invitedProfiles.length > 0) {
      const totalPassengers = event.invitedProfiles.length + 1;
      console.log('Setting passengers to:', totalPassengers);
      setNumberOfPassengers(totalPassengers);
    }
  }, [event, event?.invitedProfiles]);
  ...
  const handleConfirmDriver = async () => {
    ...
    if (newRide) {
      const transaction = await createPendingTransaction(newRide.id, finalPrice, paymentMethod.id);
  ...
  const handleOptionSelect = (option: PricingOption) => {
    console.log('Option selected:', option, 'Event connections:', eventConnections);
      ...
      <SafeAreaView style={styles.container} edges={['bottom']}>
        ...
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.optionsContent}>
          <Pressable 
            ...
            onPress={() => {
              console.log('App price pressed');
              handleOptionSelect('app_price');
            }}
          >
          ...
          <Pressable 
            ...
            onPress={() => {
              console.log('Select driver pressed');
              handleOptionSelect('select_driver');
            }}
          >
            <View style={styles.optionHeader}>
          ...
          <Pressable 
            ...
            onPress={() => {
              console.log('Name price pressed');
              handleOptionSelect('name_price');
            }}
          >
            <View style={styles.optionHeader}>
          ...
          <Pressable 
            ...
            onPress={() => {
              console.log('Receive bids pressed');
              handleOptionSelect('receive_bids');
            }}
          >
            <View style={styles.optionHeader}>
